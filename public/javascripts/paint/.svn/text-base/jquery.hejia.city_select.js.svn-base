  ...
  $.fn.extend({
    "CitySelectTag": function(options){
      ...
      function init(province_name,city_name) {
         ...
         $wrapper.append("省份:<select class='required' name=" +province_name + "><option value=''>请选择</option></select>" );
         $wrapper.append("城市:<select class='required' name=" + city_name + "><option value=''>请选择</option></select>" );
         $.getJSON('/paint_public/get_province',function(data) {
           alert(data);
      ...
      function get_city(value) {
          $.getJSON('/paint_public/get_city_json', {province: $province.val()}, function(data) {

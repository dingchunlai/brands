jQuery(document).ready(function($) {
    ...
    function uploadSuccessHandler(event, id, file, response, data) {
        url_prefix = $("#promotion_form").attr("data-assetsurl")
        $("#upload_notice").text("提示：点击右边的按钮上传图片，单图大小不能超过200K。");
        ...
        mce_insertimage(url_prefix + img_path);
    ...
    $("#swfupload_button").uploadify({
        auto        : true, // 自动开始上传文件
        script         : $("#promotion_form").attr("data-uploadurl"),

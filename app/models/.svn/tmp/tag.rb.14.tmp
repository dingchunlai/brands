...
class Tag < Hejia::Db::Hejia
  ...
  def self.get_tagname(tag_id)
    return nil unless tag_id.to_i > 0
    Tag.find(tag_id).TAGNAME rescue nil
  end
  ...
  class << self
    ...
    def zhuanqu_tags
      memkey = "Tag:tuku_indexs:zhuanqu_tags"
      tags = Rails.cache.fetch(memkey, :expires_in => 1.day) do
        Tag.find(:all,:select=>"TAGID, TAGNAME",:conditions=>"TAGID not in (42160,4401) and TAGFATHERID = 4346 and TAGESTATE='0' and TAGTYPE = 'case'")
      end
      tags
    end

    ## 取得zhuanqu_tags的子集元素集合
    def zhuanqu_children_tags
      memkey = "Tag:tuku_indexs:zhuanqu_children_tags"
      tags = Rails.cache.fetch(memkey, :expires_in => 1.day) do
        Tag.find(:all,:conditions => "TAGFATHERID in (#{Tag.zhuanqu_tags.map{|t|t.TAGID}.join(',')}) and TAGESTATE='0' and TAGTYPE = 'case'", :order => "TAGID desc").sample(150)
      end
      tags
    end
    ...
    def find_article_methods
      @find_article_methods ||= {
        ...
        '厨房电器' => {
          '文章列表1' => lambda {
            HejiaIndex.get_zhuanqu_list(2027,3,1,1)
          },
          '文章列表2' => lambda {
            HejiaIndex.get_zhuanqu_list(1891,3,1,1)
          },
          '文章列表3' => lambda {
            HejiaIndex.get_zhuanqu_list(31232,3,1,1)
          },
          '术语'      => lambda {
        ...
        '吊顶' => {
          '文章列表1' => lambda {
            HejiaIndex.find_by_keyword '集成吊顶', :select => 'title, url', :limit => 6, :entity_type_id => 1
          },
          '文章列表2' => lambda {
            HejiaIndex.find_by_keyword '石膏板吊顶', :select => 'title, url', :limit => 6, :entity_type_id => 1
          },
          '文章列表3' => lambda {
            HejiaIndex.find_by_keyword 'pvc吊顶', :select => 'title, url', :limit => 6, :entity_type_id => 1
          },
          '术语'      => lambda {
            HejiaIndex.find_by_keyword '卫生间吊顶', :select => 'title, url', :limit => 15, :entity_type_id => 1
          },
          '最新文章'  => lambda {
            #HejiaIndex.find_by_keyword '厨房吊顶', :select => 'title, url, description', :limit => 15, :entity_type_id => 1
            Article.find(:all,:conditions => ["STATE <> -1 and KEYWORD1 like ?","%吊顶%"], :order => "ID desc", :limit => 15)
            #HejiaIndex.find_by_keyword '吊顶装修知识', :select => 'title, url, description', :limit => 15, :entity_type_id => 1
          }
        }
      }
    end
    ...
    def all_categories(name = nil)
      ...
      Tag.find_by_sql "SELECT `HEJIA_TAG`.* FROM `HEJIA_TAG` JOIN HEJIA_TAGLINK as link on HEJIA_TAG.TAGID = link.TAGID2 WHERE ((TAGESTATE = '0' OR TAGESTATE = '1') AND #{condition_sql}) ORDER BY priority DESC"
    end
    ...
    def find_category(id)
      ...
      Tag.find_by_sql("SELECT `HEJIA_TAG`.* FROM `HEJIA_TAG` JOIN HEJIA_TAGLINK as link on HEJIA_TAG.TAGID = link.TAGID2 WHERE ((TAGESTATE = '0' OR TAGESTATE = '1') AND #{condition_sql}) LIMIT 1").first
    end

    #取得已经启用的品类
    def used_categories
      #Tag.categories.find(:all, :select=>"TAGID,TAGNAME,TAGURL", :conditions=>["TAGNAME not in (?)", LINK_URLS['尚未开通品类'].keys])
      Tag.categories.find(:all, :select=>"TAGID,TAGNAME,TAGURL", :conditions=>["TAGESTATE = '0'"], :order => 'priority DESC')
    end

    def get_tagid_by_tagurl(tagurl)
      Tag.categories.find_by_TAGURL(tagurl).TAGID rescue 0
    end

    def get_tagid_by_tagname(tagname)
      Tag.categories.find_by_TAGNAME(tagname).TAGID rescue 0
    end

    def get_tagname_by_tagid(tag_id)
      get_tagname(tag_id).to_s
    end

    def select_options
      Tag.all_categories.map{ |t| [t.TAGNAME, t.TAGID] }
    end

    def categories_size
      memkey = "Tag:categories:size"
      Rails.cache.fetch(memkey, :expires_in => 1.day) do
        Tag.all_categories.size
      end
    end
  ...
  # 取得某一品类下的所有品牌
  # 由于优惠券的品牌加入 ［现添 加参数1  可以选择是否包含下线的 , 参数2 也可加入按字母进行索引］ 
  def brands(include_hidden = false, init = nil)
    Brand.with_initial(init).of_tag(self.TAGID, include_hidden)
  end

class PublishContent < Hejia::Db::Hejia
  ...
  def self.zhuanti_valid_contents
    key = "publish_column_zhuanti_publish_contents"
    rs = Hejia[:cache].fetch(key, 2.hours) do
      PublishContent.zhuanti_contents
    end
    return rs
  end

  def self.valid_contents(column_id)
    key = "publish_column_#{column_id}_publish_contents"
    rs = Hejia[:cache].fetch(key, 2.hours) do
      PublishContent.single_zhuanti_contents(column_id)
    end
    return rs
  end

  named_scope :zhuanti_contents, 
    :conditions => ["publish_column_id in (?) and is_valid = ? and is_del = ?", [55698,55699,55700,55701,55702,55703], 2, false],
    :order => "order_id DESC, entity_created_at DESC"

  named_scope :single_zhuanti_contents, lambda{ |column_id|
    {
